...
- Yahoo Finance quote and historical-price refresh with Neon Postgres caching, cached-first page loads, and secured Vercel Cron refresh slots across Thai and US market hours.
...
Market refresh runs are tracked in `market_refresh_runs`. Vercel Cron calls slot-specific routes in `Asia/Bangkok` at 10:00, 12:30, 14:30, and 16:35 for the Thai market; 18:00, 19:00, and 20:00 for evening coverage; and 20:30, 21:00, 21:30, 22:00, 23:00, 00:00, 03:00, and 04:00 across US market open and close in both daylight-saving and standard time. Each route requires either `Authorization: Bearer $CRON_SECRET` from Vercel Cron or a GitHub Actions OIDC token from `.github/workflows/market-refresh.yml`, starts every portfolio through the guarded `daily-auto` path, and records one run per Bangkok day per portfolio per slot. Admin manual refresh remains available from the app and bypasses the scheduled slot limit. Manual and cron refreshes return quickly, then a protected worker processes market data in batches and updates run progress so older portfolios do not hold a browser request open until Vercel times out. Vercel Hobby cron timing is hourly best-effort, so these slots are target windows rather than exact minute guarantees; the GitHub workflow is an authenticated fallback and the run table keeps duplicate slot calls idempotent. Dashboard price freshness is based on open-position quotes, so a newer benchmark quote cannot mask stale holdings.
