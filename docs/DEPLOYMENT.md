...
- Confirm the Vercel Cron entries exist for `GET /api/cron/market-data/1000`, `/1230`, `/1430`, `/1635`, `/1800`, `/1900`, `/2000`, `/2030`, `/2100`, `/2130`, `/2200`, `/2300`, `/0000`, `/0300`, and `/0400`. These cover the Thai session, evening fallback, and US open/close across daylight-saving changes in `Asia/Bangkok`. On Vercel Hobby, cron timing is hourly best-effort, so treat these as target windows rather than exact minute triggers.
- Confirm the GitHub Actions `Market refresh fallback` workflow is enabled and the `APP_BASE_URL` repository variable points at the production origin. It authenticates with GitHub OIDC rather than a repo `CRON_SECRET`, so it keeps running even if the old GitHub keep-warm secret drifts.
